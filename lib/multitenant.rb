      ...
      before_validation Proc.new {|m|
        ...
        if tenant_id.nil? then
          ...
          if Thread.current[:unauthenticated_route]
            $logger.info(message: 'account_id was assigned by multitenant in an unauthenticated route', request_path: Thread.current[:request_path], request_domain: Thread.current[:request_domain])
          end
